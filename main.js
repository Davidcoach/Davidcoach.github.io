document.addEventListener('DOMContentLoaded', function () {
    ...
    function leadPriority(pub) {
        ...
        if (title.startsWith('NicheDeSig')) return 1;
    ...
    function sortLeadWorks(a, b) {
        ...
        const preferredCoFirstRank = pub => {
            const title = pub.title || '';
            if (title.startsWith('LLM-Inpainter')) return 0;
            if (title.startsWith('SCTrans')) return 1;
            if (title.startsWith('NicheDeSig')) return 2;
            if (title.startsWith('Diffusion-based Blemish Prior')) return 3;
            if (title.startsWith('InpaintFormer')) return 4;
            return 5;
        };
        const preferredDiff = preferredCoFirstRank(a) - preferredCoFirstRank(b);
        if (preferredDiff !== 0) return preferredDiff;
        if (b.year !== a.year) return b.year - a.year;
        return publications.indexOf(a) - publications.indexOf(b);
    }

    function sortCollaborationWorks(a, b) {
        const aIsOxygen = (a.title || '').startsWith('Oxygen TryOn');
        const bIsOxygen = (b.title || '').startsWith('Oxygen TryOn');
        if (aIsOxygen !== bIsOxygen) return aIsOxygen ? -1 : 1;
        return publications.indexOf(a) - publications.indexOf(b);
    }
    ...
    function renderSelected() {
        ...
        const collaborationPubs = selectedPubs.filter(p => p.contribution !== 'lead').sort(sortCollaborationWorks);
        pubList.innerHTML = `
